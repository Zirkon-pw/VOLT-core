...
type stubSettingsRepo struct {
	settings coresettings.AppSettings
}

func (s *stubSettingsRepo) Get() (coresettings.AppSettings, error) {
	return s.settings, nil
}

func (s *stubSettingsRepo) Save(settings coresettings.AppSettings) error {
	s.settings = settings
	return nil
}

func TestLocalizationServiceFallbacksToEnglish(t *testing.T) {
	repo := &stubSettingsRepo{settings: coresettings.AppSettings{Locale: coresettings.AutoLocale}}
...
func TestLocalizationServiceMergesCustomLocale(t *testing.T) {
	...
	repo := &stubSettingsRepo{settings: coresettings.AppSettings{Locale: "ru"}}
...
func TestLocalizationServiceFallsBackWhenSavedLocaleMissing(t *testing.T) {
	repo := &stubSettingsRepo{settings: coresettings.AppSettings{Locale: "de"}}
	service, err := NewLocalizationService(repo, filepath.Join(t.TempDir(), "locales"))
	if err != nil {
		t.Fatalf("NewLocalizationService() error = %v", err)
	}
...
func TestLocalizationServiceSetLocalePersistsSelection(t *testing.T) {
	repo := &stubSettingsRepo{settings: coresettings.DefaultAppSettings()}
	service, err := NewLocalizationService(repo, filepath.Join(t.TempDir(), "locales"))
	if err != nil {
		t.Fatalf("NewLocalizationService() error = %v", err)
	}
