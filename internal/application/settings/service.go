...
type LocalizationService struct {
	settingsRepo coresettings.Repository
	localeDir    string

	mu      sync.RWMutex
	current coresettings.LocalizationPayload
}

func NewLocalizationService(settingsRepo coresettings.Repository, localeDir string) (*LocalizationService, error) {
...
func (s *LocalizationService) GetLocalization(preferredLocales []string) (coresettings.LocalizationPayload, error) {
...
func (s *LocalizationService) SetLocale(locale string, preferredLocales []string) (coresettings.LocalizationPayload, error) {
...
func (s *LocalizationService) resolveCurrent(preferredLocales []string) (coresettings.LocalizationPayload, error) {
	current, err := s.settingsRepo.Get()
	if err != nil {
		return coresettings.LocalizationPayload{}, err
	}
	...
	return coresettings.LocalizationPayload{
...
func resolveEffectiveLocale(selectedLocale string, preferredLocales []string, catalogs map[string]localeCatalog) string {
	if selectedLocale != coresettings.AutoLocale {
...
func buildAvailableLocales(catalogs map[string]localeCatalog) []coresettings.AvailableLocale {
	...
	available := make([]coresettings.AvailableLocale, 0, len(codes))
	for _, code := range codes {
		catalog := catalogs[code]
		available = append(available, coresettings.AvailableLocale{
			Code:   code,
			Label:  catalog.Label,
...
func normalizeSelectedLocale(locale string) string {
	...
	if normalized == "" {
		return coresettings.AutoLocale
	}
	return normalized
}
...
func cloneLocalizationPayload(payload coresettings.LocalizationPayload) coresettings.LocalizationPayload {
	clonedAvailable := make([]coresettings.AvailableLocale, len(payload.AvailableLocales))
...
func (s *LocalizationService) setCurrent(payload coresettings.LocalizationPayload) {
	s.mu.Lock()
	defer s.mu.Unlock()
