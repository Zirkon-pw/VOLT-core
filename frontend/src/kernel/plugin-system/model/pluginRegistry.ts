...
} from '@kernel/plugin-system/runtime';
...
interface PluginRegistryState {
  ...
  registerSettingsPage: (page: RegisteredSettingsPage) => void;
...
function runSettingsPageCleanup(page: RegisteredSettingsPage): void {
  if (!page.cleanup) {
    return;
  }
  ...
  try {
    ...
    reportPluginError(page.pluginId, `settingsPage:${page.id}:cleanup`, err);
  }
}
...
export const usePluginRegistryStore = create<PluginRegistryState>((set) => ({
  ...
  registerSettingsPage: (page) => set((s) => ({ settingsPages: [...s.settingsPages, page] })),
  ...
  removeByPluginId: (pluginId) => set((s) => {
    ...
    const removedSettingsPages = s.settingsPages.filter((page) => page.pluginId === pluginId);
    removedPages.forEach((page) => runPageCleanup(page, true));
    removedSettingsPages.forEach((page) => runSettingsPageCleanup(page));
    ...
    return {
      ...
      settingsPages: s.settingsPages.filter((page) => page.pluginId !== pluginId),
  ...
  clearAll: () => set((s) => {
    ...
    s.settingsPages.forEach((page) => runSettingsPageCleanup(page));
...
export function registerSettingsPage(page: RegisteredSettingsPage): void {
  usePluginRegistryStore.getState().registerSettingsPage(page);
}
